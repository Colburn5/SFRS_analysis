...
class PCFS:
    ...
    def __init__(self, folder_path ,measurement_mode, simulation = False, memory_limit = 1, header_lines_pcfslog = 5):
        ...
        if simulation:
            ...
            file_list = [f.replace('.1.ttbin', '') for f in glob.glob('*.1.ttbin')] # without extension
            
            numbered_files = [(extract_number(file), file) for file in file_list]

            numbered_files.sort()
            
            # Extract the sorted list of strings
            self.file_stream = [file for number, file in numbered_files]
    ...
    '''
    def get_splev_mirror_spec_corr(self, white_fringe_pos, white_fringe_ind,fit_interferogram = False,stage_increment=0.005):

        # construct mirrored data
        interferogram = self.blinking_corrected_PCFS_interferogram[:,:]
        mirror_intf = np.hstack((np.fliplr(interferogram[:, white_fringe_ind:]), interferogram[:, white_fringe_ind+1:]))
        temp = white_fringe_pos - self.stage_positions[white_fringe_ind:]
        temp2  = self.stage_positions[white_fringe_ind+1:]-white_fringe_pos
        print(f'temp = {temp}')
        mirror_stage_pos = np.hstack((temp2[::-1],temp))
        interp_stage_pos = np.arange(min(mirror_stage_pos), max(mirror_stage_pos)+stage_increment, stage_increment )
        print( f'mirror = {mirror_stage_pos}')
        # row-wise interpolation
        a,b = mirror_intf.shape
        interp_mirror = np.zeros((a,len(interp_stage_pos)))
        for i in range(a):
            ...
            invalid_indices = ~np.isfinite(ynew)
            
            ynew[invalid_indices] = 0
        
            if fit_interferogram:
                
                exponential_decay_fit = lambda x, a, b: a * np.exp(-b * np.abs(x))
                try:
                    popt, pcov = curve_fit(exponential_decay_fit, xnew, ynew)
                    a_fit, b_fit = popt
                    interp_mirror[i,:] = exponential_decay_fit(xnew, a_fit, b_fit)
                except ValueError:
